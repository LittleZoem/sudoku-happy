...
class NPOperator extends StrategyOperator
{
    constructor() {
        super("NPOperator");
        this.cellMap = null;
...
class NPStratrgy extends Strategy
{
    constructor() {
        super("Hidden Singles", new NPOperator(), [true, true, true]);
    ...
    handle (candidates, cellTrack, strategyUnits) {
        ...
        const candidateHandle = (y, x, pair) => {
            ...
            if (candidates[y][x].length !== 0 && (y !== pair[2] || x !== pair[3]) && (y !== pair[0] || x !== pair[1]) ) {
                const index1 = candidates[y][x].indexOf(pair[4]);
                const index2 = candidates[y][x].indexOf(pair[5]);
