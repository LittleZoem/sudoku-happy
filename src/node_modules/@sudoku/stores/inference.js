...
import { roam, GridState } from '@sudoku/stores/roam';
...
function createInferenceGrid() {
	...
	return {
        ...
        set: (pos, value) => {
            ...
            roam.record(new GridState(userGrid.saveState(), pos, value));

            if (get(candidateGrid)[pos.y][pos.x].length > 1) {
                if (!get(visitedNums)[pos.x + ',' + pos.y] || get(visitedNums)[pos.x + ',' + pos.y].length + 1 < get(candidateGrid)[pos.y][pos.x].length) {
                    roam.save(pos, false);
                } else {
                    roam.save(pos, true);
                }
        ...
        showInferece: (x, y) => {
            ...
            strategyTrack.set(strategyLink);
        },
	};
}
